async function loadProducts(filter = 'all') {
    try {
        ...
        container.innerHTML = products.map(p => {
            const title = p.name || p.title || 'Потужний товар';
            const price = p.price || 0;
            const image = p.image || 'https://images.unsplash.com/photo-1521572267360-ee0c2909d518?w=600&auto=format&fit=crop&q=80';
            const desc = p.description || 'Стильний елемент колекції';
